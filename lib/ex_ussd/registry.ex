defmodule ExUssd.Registry do
  ...
  def add_attempt(session, text), do: GenServer.call(via_tuple(session), {:add_attempt, text})
  ...
  def reset_attempt(session), do: GenServer.call(via_tuple(session), {:reset_attempt})
  ...
  def handle_call(
        {:add_attempt, text},
        _from,
        %State{route: [%{attempt: %{count: count, inputs: inputs}} = head | tail]} = state
      ) do
    attempt = %{count: count + 1, inputs: [text | inputs]}
    new_state = [%{head | attempt: attempt} | tail]

    {:reply, new_state, Map.put(state, :route, new_state)}
  end
  ...
  def handle_call({:route_back}, _from, %State{route: [%{depth: depth} = head | tail]} = state) do
    if head[:depth] == 1 do
      route = with [] <- tail, do: [%ExUssd.Route.State{depth: 1, text: "555"}]
  ...
  def handle_call({:reset_attempt}, _from, %State{route: [head | tail]} = state) do
    attempt = %{count: 0, inputs: []}
    new_state = [%{head | attempt: attempt} | tail]
