defmodule ExUssd.Display do
  def generate(menu: menu, routes: routes) do
    %{title: title, error: error, split: split, menu_list: menu_list, show_options: show_options, next: next, previous: previous, should_close: should_close, display_style: display_style} = menu
    %{depth: page} = hd(routes)

    { min, max } = case show_options do
      false -> {(split * (page - 1)) + 1000000, (page * split) + 1000000}
      true -> {(split * (page - 1)), (page * split) - 1}
    end

    selection = Enum.into min..max, []
    positions = selection |> Enum.with_index()
    menus = Enum.map(positions, fn(x) ->
    ...
    previous_navigation = case length(routes) do
      1 -> case page do
        1 -> ""
    ...
    next_navigation = case Enum.at(menu_list, max + 1) do
      nil -> ""
      _->
        case show_options do
          false -> ""
